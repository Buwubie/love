...
int w_newImageData(lua_State *L) {
  ...
  if (lua_isnumber(L, 1)) {
    ...
    if (w <= 0 || h <= 0) return luaL_error(L, "Invalid image size.");
    ...
    if (bytes) {
      if (numbytes != t->getSize()) {
        t->release();
        return luaL_error(L,
                          "The size of the raw byte string must match the "
  ...
  } else if (filesystem::luax_cangetdata(L, 1))  // Case 2: File(Data).
  {
    ...
    ImageData *t = nullptr;
    luax_catchexcept(
        L, [&]() { t = instance()->newImageData(data); },
        [&](bool) { data->release(); });
...
int w_newCompressedData(lua_State *L) {
  ...
  luax_catchexcept(
      L, [&]() { t = instance()->newCompressedData(data); },
      [&](bool) { data->release(); });
...
int w_newCubeFaces(lua_State *L) {
  ...
  for (auto face : faces) luax_pushtype(L, face);
  return (int)faces.size();
}

// List of functions to wrap.
static const luaL_Reg functions[] = {{"newImageData", w_newImageData},
                                     {"newCompressedData", w_newCompressedData},
                                     {"isCompressed", w_isCompressed},
                                     {"newCubeFaces", w_newCubeFaces},
                                     {0, 0}};

static const lua_CFunction types[] = {luaopen_imagedata,
                                      luaopen_compressedimagedata, 0};
