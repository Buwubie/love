...
CompressedImageData::CompressedImageData(
    const std::list<FormatHandler *> &formats, Data *filedata)
...
CompressedImageData::CompressedImageData(const CompressedImageData &c)
  ...
  for (const auto &i : c.dataImages) {
    auto slice =
        new CompressedSlice(i->getFormat(), i->getWidth(), i->getHeight(),
                            memory, i->getOffset(), i->getSize());
...
size_t CompressedImageData::getSize() const { return memory->getSize(); }

void *CompressedImageData::getData() const { return memory->getData(); }

int CompressedImageData::getMipmapCount() const {
  return (int)dataImages.size();
}

int CompressedImageData::getSliceCount(int /*mip*/) const { return 1; }
...
PixelFormat CompressedImageData::getFormat() const { return format; }

void CompressedImageData::setLinear(bool linear) {
  for (auto &slice : dataImages) slice->setLinear(linear);
}
