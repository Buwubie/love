...
Image::Image() : Module(M_IMAGE, "love.image.magpie") {
  using namespace magpie;

  float16Init();  // Makes sure half-float conversions can be used.

  formatHandlers = {
      new PNGHandler, new STBHandler, new EXRHandler, new DDSHandler,
...
Image::~Image() {
  ...
  for (FormatHandler *handler : formatHandlers) handler->release();
}
...
bool Image::isCompressed(Data *data) {
  for (FormatHandler *handler : formatHandlers) {
    if (handler->canParseCompressed(data)) return true;
  }
...
ImageData *Image::newPastedImageData(ImageData *src, int sx, int sy, int w,
                                     int h) {
...
std::vector<StrongRef<ImageData>> Image::newCubeFaces(
    love::image::ImageData *src) {
...
std::vector<StrongRef<ImageData>> Image::newVolumeLayers(ImageData *src) {
  ...
  if (totalW % totalH == 0) {
    for (int i = 0; i < totalW / totalH; i++)
      layers.emplace_back(
          newPastedImageData(src, i * totalH, 0, totalH, totalH),
          Acquire::NORETAIN);
  } else if (totalH % totalW == 0) {
    for (int i = 0; i < totalH / totalW; i++)
      layers.emplace_back(
          newPastedImageData(src, 0, i * totalW, totalW, totalW),
          Acquire::NORETAIN);
  } else
