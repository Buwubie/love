...
class ImageData : public ImageDataBase {
 public:
  ...
  /**
   * Paste part of one ImageData onto another. The subregion defined by the
   *top-left corner (sx, sy) and the size (sw,sh) will be pasted to (dx,dy) in
   *this ImageData.
  ...
  /**
   * Checks whether a position is inside this ImageData. Useful for checking
   *bounds.
   ...
   **/
  love::filesystem::FileData *encode(FormatHandler::EncodedFormat format,
                                     const char *filename,
 ...
 private:
  ...
  static StringMap<FormatHandler::EncodedFormat,
                   FormatHandler::ENCODED_MAX_ENUM>::Entry
      encodedFormatEntries[];
  static StringMap<FormatHandler::EncodedFormat,
                   FormatHandler::ENCODED_MAX_ENUM>
      encodedFormats;
