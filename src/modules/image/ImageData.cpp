...
ImageData::ImageData(int width, int height, PixelFormat format, void *data,
                     bool own)
    : ImageDataBase(format, width, height) {
  if (!validPixelFormat(format))
    throw love::Exception("ImageData does not support the %s pixel format.",
...
void ImageData::decode(Data *data) {
  ...
  if (module == nullptr)
    throw love::Exception(
        "love.image must be loaded in order to decode an ImageData.");
  ...
  if (decodedimage.size != getPixelFormatSliceSize(decodedimage.format,
                                                   decodedimage.width,
                                                   decodedimage.height)) {
...
love::filesystem::FileData *ImageData::encode(
    FormatHandler::EncodedFormat encodedFormat, const char *filename,
  ...
  if (module == nullptr)
    throw love::Exception(
        "love.image must be loaded in order to encode an ImageData.");
  ...
  if (writefile) {
    ...
    if (fs == nullptr) {
      filedata->release();
      throw love::Exception(
          "love.filesystem must be loaded in order to write an encoded "
          "ImageData to a file.");
    }
...
void ImageData::setPixel(int x, int y, const Colorf &c) {
  ...
  if (pixelSetFunction == nullptr)
    throw love::Exception(
        "ImageData:setPixel does not currently support the %s pixel format.",
        getPixelFormatName(format));
...
void ImageData::getPixel(int x, int y, Colorf &c) const {
  ...
  if (pixelGetFunction == nullptr)
    throw love::Exception(
        "ImageData:getPixel does not currently support the %s pixel format.",
        getPixelFormatName(format));
...
void ImageData::paste(ImageData *src, int dx, int dy, int sx, int sy, int sw,
                      int sh) {
  ...
  if (sx >= srcW || sx + sw < 0 || sy >= srcH || sy + sh < 0 || dx >= dstW ||
      dx + sw < 0 || dy >= dstH || dy + sh < 0)
    return;
  ...
  if (srcformat == dstformat &&
      (sw == dstW && dstW == srcW && sh == dstH && dstH == srcH)) {
    ...
    for (int i = 0; i < sh; i++) {
      ...
      if (srcformat == dstformat)
      ...
      else if (srcformat == PIXELFORMAT_RGBA8_UNORM &&
               dstformat == PIXELFORMAT_RGBA16_UNORM)
        pasteRGBA8toRGBA16(rowsrc, rowdst, sw);
      else if (srcformat == PIXELFORMAT_RGBA8_UNORM &&
               dstformat == PIXELFORMAT_RGBA16_FLOAT)
        pasteRGBA8toRGBA16F(rowsrc, rowdst, sw);
      else if (srcformat == PIXELFORMAT_RGBA8_UNORM &&
               dstformat == PIXELFORMAT_RGBA32_FLOAT)
        pasteRGBA8toRGBA32F(rowsrc, rowdst, sw);

      else if (srcformat == PIXELFORMAT_RGBA16_UNORM &&
               dstformat == PIXELFORMAT_RGBA8_UNORM)
      ...
      else if (srcformat == PIXELFORMAT_RGBA16_FLOAT &&
               dstformat == PIXELFORMAT_RGBA8_UNORM)
      ...
      else if (srcformat == PIXELFORMAT_RGBA32_FLOAT &&
               dstformat == PIXELFORMAT_RGBA8_UNORM)
        ...
        throw love::Exception(
            "ImageData:paste does not currently support converting from the %s "
            "pixel format.",
            getPixelFormatName(srcformat));
      else
        throw love::Exception(
            "ImageData:paste does not currently support converting to the %s "
            "pixel format.",
            getPixelFormatName(dstformat));
    }
  }
}
...
StringMap<FormatHandler::EncodedFormat, FormatHandler::ENCODED_MAX_ENUM>::Entry
    ImageData::encodedFormatEntries[] = {
        {"tga", FormatHandler::ENCODED_TGA},
        {"png", FormatHandler::ENCODED_PNG},
        {"exr", FormatHandler::ENCODED_EXR},
};

StringMap<FormatHandler::EncodedFormat, FormatHandler::ENCODED_MAX_ENUM>
    ImageData::encodedFormats(ImageData::encodedFormatEntries,
                              sizeof(ImageData::encodedFormatEntries));
