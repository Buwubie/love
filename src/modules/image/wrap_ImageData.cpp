...
int w_ImageData_getFormat(lua_State *L) {
  ...
  if (!getConstant(format, fstr)) return luaL_error(L, "Unknown pixel format.");
...
int w_ImageData_setPixel(lua_State *L) {
  ...
  if (lua_istable(L, 4)) {
    for (int i = 1; i <= components; i++) lua_rawgeti(L, components, i);

    c.r = (float)luaL_checknumber(L, -components);
    if (components > 1) c.g = (float)luaL_checknumber(L, (-components) + 1);
    if (components > 2) c.b = (float)luaL_checknumber(L, (-components) + 2);
    if (components > 3) c.a = (float)luaL_optnumber(L, (-components) + 3, 1.0);

    lua_pop(L, components);
  } else {
    c.r = (float)luaL_checknumber(L, 4);
    if (components > 1) c.g = (float)luaL_checknumber(L, 5);
    if (components > 2) c.b = (float)luaL_checknumber(L, 6);
    if (components > 3) c.a = (float)luaL_optnumber(L, 7, 1.0);
  }
...
int w_ImageData_mapPixel(lua_State *L) {
  ...
  for (int y = sy; y < sy + h; y++) {
    for (int x = sx; x < sx + w; x++) {
      ...
      c.r = (float)luaL_checknumber(L, -4);
      if (components > 1) c.g = (float)luaL_checknumber(L, -3);
      if (components > 2) c.b = (float)luaL_checknumber(L, -2);
      if (components > 3) c.a = (float)luaL_optnumber(L, -1, 1.0);
...
int w_ImageData_encode(lua_State *L) {
  ...
  if (!ImageData::getConstant(fmt, format))
    return luax_enumerror(L, "encoded image format",
                          ImageData::getConstants(format), fmt);
  ...
  luax_catchexcept(L, [&]() {
    filedata = t->encode(format, filename.c_str(), hasfilename);
  });
...
static FFI_ImageData ffifuncs = {
    float16to32, float32to16, float11to32,
    float32to11, float10to32, float32to10,
};
...
extern "C" int luaopen_imagedata(lua_State *L) {
  int ret = luax_register_type(L, &ImageData::type, data::w_Data_functions,
                               w_ImageData_functions, nullptr);

  love::data::luax_rundatawrapper(L, ImageData::type);
  luax_runwrapper(L, imagedata_lua, sizeof(imagedata_lua), "ImageData.lua",
                  ImageData::type, &ffifuncs);
